...
- Fixed issue where Task can't be cancelled from UI. Change enables users to terminate the transfer even if file upload
  is ongoing.
...
- [Breaking] New feature: Added parameter AssumeFileExistence which will when enabled, skip checking if destination file
  esists and overwrites any existing files automatically.
